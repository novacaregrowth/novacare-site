...
- ClosingArc (merged closing arc combining what was framed as Beat 5 plus Beat 6 into one component, src/components/sections/ClosingArc.tsx). Top region: four "Why Novacare" pillars in a 2x2 grid on desktop, single column on mobile, anchored by Fraunces serif numerals 01-04 in weight 300 at 36/48px stone-soft. Patek-restrained header with no visible H2, just the WHY NOVACARE caption with terracotta hairline. Per-pillar useScroll plus useTransform fades each body between 0.3 and 1.0 opacity as the pillar passes through viewport center; headings and numerals stay solid. Cascade: PRE_PILLARS_DELAY_S 0.8s plus 0.4s pillar-to-pillar stagger, total ~3.7s. Visually-hidden h2 with id why-heading provides landmark naming via aria-labelledby. Bottom region: closing CTA with terracotta hairline plus capacity caption "Three slots remaining this quarter.", H2 "Built for the practice you've earned." (word-by-word reveal at HEADLINE_START_S 0.7s, 0.08s word stagger), subhead, single primary WhatsApp CTA, discreet email caption with terracotta scaleX hover underline. One <video> element rendering /beat-6-amber-smoke.mp4 spans the smoke emergence zone and the closing CTA region. Single decoder eliminates the frame-async tear that occurred when WhyNovacare and ClosingCTA each ran their own video instance. Three gradient overlays in the smoke region: top-fade-from-ink (smoke emergence below pillars), left-to-right wash (text contrast for the closing region), bottom-fade-to-ink (page-end settle). Two useInView triggers, pillars on the inner pillars section and closing on the inner content div, preserve the original Beat 5 and Beat 6 reveal cadences. Full reduced-motion path with static smoke poster.
- SmoothScroll provider wired globally via Lenis in layout.tsx
- shadcn/ui initialized with Card primitive
- copy.ts contains hero, phoneScene, aiAnswerScene, services with v4 shape (sectionLabel, headline, subhead, tiers with introHeader, dashboard data on tier3, addon), why (label, four points with title and body), and closing (capacity caption, H2, subhead, primary WhatsApp CTA, secondary email)
...
Next: Mobile audit. Beat 3 (Al Yasmine case study) remains postponed.
...
13. When two adjacent sections share a single ambient asset (video, large image, gradient field), render the asset in one element across both regions rather than two synchronized instances. Two <video> decoders running independent clocks will frame-async, producing a visible tear at the seam regardless of how matched the gradient stitching is. ClosingArc was born from collapsing WhyNovacare plus ClosingCTA into one component for exactly this reason.
